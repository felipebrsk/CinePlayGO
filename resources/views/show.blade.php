...
@section('content')
	<div class="movie-info border-b border-gray-800">
		<div class="container mx-auto px-4 py-16 flex flex-col md:flex-row">
			<img alt="Kung Fu Panda 2" src="{{ 'https://image.tmdb.org/t/p/w342/' . $popularMovies['poster_path'] }}" class="w-64 md:w-96">
			<div class="md:ml-24">
				<h2 class="text-4xl font-semibold">{{ $popularMovies['title'] }}</h2>
				<div class="flex flex-wrap items-center text-gray-400 text-sm mt-2">
					...
					@if(isset($popularMovies['release_date']))
                							<span>{{ \Carbon\Carbon::parse($popularMovies['release_date'])->format('d M, Y') }}</span>
                							@else
				...
				<p class="text-gray-300 mt-8">{{ $popularMovies['overview'] }} {{ $popularMovies['tagline'] }}</p>
				...
				<div class="mt-12">
                    ...
                    @else
                    <div class="font-semibold mt-16">Trailer indisponível.</div>
                    @endif
				</div>
			</div>
		</div>
...
<div class="movie-cast border-b border-gray-800">
    ...
    <div class="movie-images" x-data="{ isOpen: false, image: ''}">
        <div class="container mx-auto px-4 py-16">
            <h2 class="text-4xl font-semibold">Imagens de {{$popularMovies['title']}}</h2>
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-8">
            @foreach($popularMovies['images']['backdrops'] as $images)
            	@if($loop->index < 10)
                    <div class="mt-8">
                        <a
                            @click.prevent="
...
<div class="container mx-auto px-4 pt-16">
	<div class="now-playing-movies py-24">
		<h2 class="uppercase tracking-wider text-orange-500 text-lg font-semibold">Filmes similares</h2>
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-8">
                @foreach($similarMovies as $similar)
                	<div class="mt-8" style=width:260px;>
                		<a href="{{route('filmes.show', $similar['id'])}}">
                			...
                			<div class="mt-2">
                				<a href="{{route('filmes.show', $similar['id'])}}" class="text-lg mt-2 hover:text-gray:300">{{ $similar['title'] }}</a>
